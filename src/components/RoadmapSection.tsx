          ...
          <div className="space-y-6 sm:space-y-8">
            {milestones.map((milestone, index) => (
              <div key={index} className="relative flex flex-col md:flex-row items-start gap-4 sm:gap-6">
                ...
                {/* Content */}
                <div className="flex-1 bg-white/20 backdrop-blur-sm border border-white/30 rounded-2xl p-6 sm:p-8 shadow-xl hover:shadow-2xl transition-all duration-300">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-3 sm:mb-4">
                    <h3 className="text-lg sm:text-xl md:text-2xl font-semibold text-white leading-tight drop-shadow-lg">
                      {milestone.title}
                    </h3>
                    <span className="font-semibold text-sm sm:text-base mt-1 sm:mt-0 text-yellow-300 drop-shadow-md">
                      {milestone.quarter}
                    </span>
                  </div>
